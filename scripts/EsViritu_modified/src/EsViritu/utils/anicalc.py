...
def parse_blast(handle):
	"""Yield dict records from BLAST tabular (outfmt 6) lines."""
	for line in handle:
		r = line.split()
		yield {
			'qname':r[0],
...
def yield_alignment_blocks(handle):
	"""Group BLAST records into blocks by (qname, tname)."""
...
def prune_alns(alns, min_len=0, min_evalue=1e-3, min_pid=90):
	"""Filter alignments by length, e-value, and percent identity."""
	# remove short aligns
...
def compute_cov(alns):
	"""Compute query and target coverage (%) from alignment blocks."""
...
def parse_arguments():
	"""Parse CLI arguments for ANI calculation script."""
